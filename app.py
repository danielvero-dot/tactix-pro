...
st.set_page_config(
    page_title="Tactix Pro - Quick Select", 
    layout="wide", 
...
interval_ideal = menit_per_pemain / 2
...
if menu_aktif == "1. Attendance Check":
    ...
    st.write("Check the players who are present at the field today:")
...
# ==========================================
    ...
    if total_hadir < format_game:
        st.error(f"Not enough players for a {format_game}v{format_game} match! Please adjust attendance on Screen 1.")
    else:
        # A. Assign GK
        ...
        # B. Assign Starter Using Quick Tap Buttons
        ...
        if jumlah_terpilih == format_game:
            st.success(f"🎉 Perfect! {jumlah_terpilih} Starters Selected.")
        elif jumlah_terpilih > format_game:
            st.error(f"🚨 Too many players! {jumlah_terpilih} selected. Deselect {jumlah_terpilih - format_game} player(s).")
        else:
            st.warning(f"📋 Need {format_game - jumlah_terpilih} more player(s) to complete the starting lineup.")
        ...
        for i, p in enumerate(st.session_state.pemain_hadir):
            ...
            with target_col:
                ...
                if st.button(label_tombol, key=f"btn_choose_{p}"):
                    if is_starter:
                        st.session_state.pemain_di_lapangan.remove(p)
                    else:
                        st.session_state.pemain_di_lapangan.append(p)
                    st.rerun()
        ...
        if len(st.session_state.pemain_di_lapangan) == format_game:
            if st.session_state.kiper_terpilih != "-- Select Goalkeeper --" and st.session_state.kiper_terpilih not in st.session_state.pemain_di_lapangan:
                st.error(f"🚨 Main Goalkeeper ({st.session_state.kiper_terpilih}) must be tapped/selected as part of the Starting Lineup!")
            else:
                st.markdown(f"<p style='color:#10b981; font-weight:bold;'>Lineup Locked: {', '.join(st.session_state.pemain_di_lapangan)}</p>", unsafe_allow_html=True)
...
# ==========================================
    ...
    if len(st.session_state.pemain_di_lapangan) != format_game:
        st.error("🚨 Lineup size mismatch! Please set your starting lineup on Screen 2 until the count is perfect.")
    else:
        ...
        with col_m2: st.metric("TARGET MINS / OUTFIELD PLAYER", f"{menit_per_pemain:.1f} Mins")
        with col_m3: st.metric("ROTATE PLAYERS EVERY", f"{interval_ideal:.1f} Mins")

        # --- SMART ROTATION ALERT ---
        if current_elapsed > 0:
            sisa_ke_jendela = current_elapsed % interval_ideal
            if sisa_ke_jendela < 1.0 and current_elapsed < total_menit:
                st.warning(f"🚨 **COACH! TIME TO ROTATE PLAYERS!** Match is at **{current_elapsed:.1f} mins**. Ideal rotation window (**{interval_ideal:.1f} mins**) reached. Swap players below!")
        ...
        with col_in: 
            daftar_pemain_masuk = st.multiselect("🟢 Entering (From Bench):", options=pemain_di_cadangan, key="live_multi_in")
        with col_out:
        ...
        if tambah_log:
            if len(daftar_pemain_masuk) > 0 and len(daftar_pemain_keluar) > 0:
                if len(daftar_pemain_masuk) == len(daftar_pemain_keluar):
                    ...
                    st.error("🚨 Substitution mismatch! The number of players entering and exiting must be equal.")

        # PROGRESS VISUAL MONITOR
        st.write("---")
        ...
        for p in st.session_state.pemain_hadir:
            ...
            col_pname, col_pbar = st.columns([1, 3])
            with col_pname:
                st.markdown(f"**{p}** <br><span style='color:#94a3b8; font-size:11px;'>{status_badge}</span>", unsafe_allow_html=True)
            with col_pbar:
